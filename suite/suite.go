...
func RunStoreSuite(t *testing.T, newStore StoreFactory) {
	...
	// Start event is only allowed for new runs
	...
	t.Run("OtherEventsOnlyForExistingRuns", func(t *testing.T) {
		...
		_, err := s.RecordEvent(ctx, nonExistentRunID, 0, events.NewCallEvent("fn", nil))
		require.Error(t, err)
	...
	t.Run("NothingAllowedAfterFinish", func(t *testing.T) {
		...
		_, err := s.RecordEvent(ctx, runID, 0, events.NewStartEvent("script-hash", nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID, 1, events.NewFinishEvent(nil, nil))
		require.NoError(t, err)

		// Try to record another event after finish
		_, err = s.RecordEvent(ctx, runID, 2, events.NewCallEvent("fn", nil))
		require.Error(t, err)
	...
	t.Run("SequentialEventIDExceptResume", func(t *testing.T) {
		...
		_, err = s.RecordEvent(ctx, runID, 5, events.NewCallEvent("fn", nil))
		require.ErrorIs(t, err, starflow.ErrConcurrentUpdate)

		// Correct sequential ID should work
		_, err = s.RecordEvent(ctx, runID, 1, events.NewCallEvent("fn", nil))
		require.NoError(t, err)
	})
	...
	t.Run("OnlyYieldEventsCanBeResumed", func(t *testing.T) {
		...
		_, err = s.RecordEvent(ctx, runID, 2, events.NewResumeEvent("signal", output))
		require.Error(t, err)
	...
	t.Run("SignalIDMustMatchYieldEvent", func(t *testing.T) {
		...
		_, err = s.RecordEvent(ctx, runID, 1, events.NewCallEvent("fn", nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID, 2, events.NewYieldEvent("signal1", runID))
		require.NoError(t, err)

		// Try to resume with wrong signal ID
		_, err = s.RecordEvent(ctx, runID, 3, events.NewResumeEvent("signal2", output))
		require.Error(t, err)
		require.Contains(t, err.Error(), "signal ID mismatch")

		// Correct signal ID should work
		_, err = s.RecordEvent(ctx, runID, 3, events.NewResumeEvent("signal1", output))
		require.NoError(t, err)
	})
	...
	t.Run("OnlyCallEventsCanBeYielded", func(t *testing.T) {
		...
		_, err = s.RecordEvent(ctx, runID, 1, events.NewYieldEvent("signal", runID))
		require.Error(t, err)
	...
	t.Run("OnlyCallEventsCanBeReturned", func(t *testing.T) {
		...
		_, err = s.RecordEvent(ctx, runID, 1, events.NewReturnEvent(nil, nil))
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid event type")
	})
	...
	t.Run("OnlyYieldOrReturnAfterCall", func(t *testing.T) {
		...
		_, err = s.RecordEvent(ctx, runID, 2, events.NewCallEvent("fn2", nil))
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid event type")

		// Try to record finish event after call (should fail)
		_, err = s.RecordEvent(ctx, runID, 2, events.NewFinishEvent(nil, nil))
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid event type")

		// Yield should work
		runID2 := shortuuid.New()
		_, err = s.RecordEvent(ctx, runID2, 0, events.NewStartEvent("script-hash", nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID2, 1, events.NewCallEvent("fn", nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID2, 2, events.NewYieldEvent("signal", runID2))
		require.NoError(t, err)

		// Return should work
		runID3 := shortuuid.New()
		_, err = s.RecordEvent(ctx, runID3, 0, events.NewStartEvent("script-hash", nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID3, 1, events.NewCallEvent("fn", nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID3, 2, events.NewReturnEvent(nil, nil))
		require.NoError(t, err)
	})
	...
	t.Run("OnlyResumeAfterYield", func(t *testing.T) {
		...
		_, err = s.RecordEvent(ctx, runID, 1, events.NewCallEvent("fn", nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID, 2, events.NewYieldEvent("signal", runID))
		require.NoError(t, err)

		// Try to record call event after yield (should fail)
		_, err = s.RecordEvent(ctx, runID, 3, events.NewCallEvent("fn2", nil))
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid event type")

		// Try to record return event after yield (should fail)
		_, err = s.RecordEvent(ctx, runID, 3, events.NewReturnEvent(nil, nil))
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid event type")

		// Try to record finish event after yield (should fail)
		_, err = s.RecordEvent(ctx, runID, 3, events.NewFinishEvent(nil, nil))
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid event type")
	})
	...
	t.Run("GetEvents", func(t *testing.T) {
		...
		_, err := s.RecordEvent(ctx, runID, 0, events.NewStartEvent("script-hash", nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID, 1, events.NewCallEvent("fn1", nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID, 2, events.NewReturnEvent(nil, nil))
		require.NoError(t, err)
		_, err = s.RecordEvent(ctx, runID, 3, events.NewFinishEvent(nil, nil))
		require.NoError(t, err)
